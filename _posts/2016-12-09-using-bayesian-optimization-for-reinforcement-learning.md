...
* $s_{t+1}$ is the new state observed. In a deterministic environment, it is a function of $s_t$ and $a_t$.
...
While there are many tunable hyperparameters in the realm of reinforcement learning and deep Q-networks, for this blog post the following 7 parameters were selected:
...
We compared the results of SigOpt’s Bayesian optimization to two standard hyperparameter tuning methods: grid search and random search. 128 objective evaluations for each optimization method were run, and we took the median of 5 runs.
